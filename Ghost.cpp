...
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, Ghost& i_ghost_0, Pacman& i_pacman)
{
	...
	//We used the modulo operator in case the gohst goes outside the grid.
	...
	{
		//we used 4 because using a number between 0 and 3 will make the gohst move in a direction it can't move.
